    ...
    <div class="wrapper">
      ...
      <div class="container">
        ...
        <form class="form">
          ...
          <input type="password" placeholder="Affirm Password" />
          <button type="button" @click="submitHandle">Sign Up</button>
        </form>
      </div>
...
export default {
  ...
  methods: {
    submitHandle() {
      let { name, email, password } = this.formData;
      if (Validate.emailValidate(email)) {
        this.alertTemplate(Validate.emailValidate(email));
        return;
      }
      if (Validate.nameValidate(name)) {
        this.alertTemplate(Validate.nameValidate(name));
        return;
      }
      if (Validate.passwordValidate(password)) {
        this.alertTemplate(Validate.passwordValidate(password));
        return;
      }
      if (
        ...
        api
          ...
          .then(res => {
            if (res.code === 1) {
              this.alertTemplate(res.msg, true);
              return;
            }
            console.log(res);
          })
